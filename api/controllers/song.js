	...
	song.save((err,songStored) => 
		{
			if (err)
			 ...
			 } else 
			 {
			 	if(!songStored)
			 	{
			 		res.status(404).send({message: 'No se ha guardado en el servidor'});
			 	}
			 	else
			 	 {
	...
	if(err)
		{
			...
			if (!songUpdated) {
				res.status(404).send({message: 'No se ha actulizado la canción'});
			} else {
	...
	if (req.files) 
		{
			...
			if (file_ext=='mp3' || file_ext == 'ogg') 
				{
					Song.findByIdAndUpdate(songId, {file : file_name}, (err, songUpated) => 
					{
						if (err) 
						{
							...
							if (!songUpated) {
								...
								res.status(200).send({song : songUpated});	
							}
						}
					});
				} 
